...
from po_translator.translation_management.data_processors import (po, csv_file,
                                                                  android_xml)
...
class TestOneSetOneLang(TestPoTranslate):
    ...
    def test_get_message_list(self):
        mes = get_message_list(self.proj.id, self.proj_lang.id)
...
class TestOneSetTwoLang(TestPoTranslate):
    ...
    def test_get_message_list(self):
        mes = get_message_list(self.proj.id, self.proj_lang.id)
        self.assertEqual(len(mes), 4)
        mes = get_message_list(self.proj.id, self.new_lang.id)
...
class TestSaveNewMeaning(TestPoTranslate):
    ...
    def test_save_new_meaning_new_set(self):
        ...
        self.assertEqual(target_messages.order_by('-id')[1].message_set,
                         previous_set)
        self.assertEqual(target_messages.order_by('-id')[0].message_set,
                         last_set)
        message_list = get_message_list(self.proj.id, self.proj_lang.id)
...
class TestFilterMessagesBySubstring(TestPoTranslate):
    ...
    def test_import_source_lang(self):
        ...
        self.assertEqual(len(mes), 2)
        self.assertEqual(set(i['msg_id'] for i in mes),
                         set(['mes.ssage3', 'messs.age4']))
...
class TestDisplayDevs(TestPoTranslate):
    ...
    def test_import_source_lang_2(self):
        ...
        for i in project_source_messages.filter(lang=self.proj.lang,
    ...
    def test_import_export_android_file(self):
        ...
        self._setup_project('android_project',
                                      'android',
                                      dict(name='English', code='en'))
    ...
    def test_import_export_csv_file(self):
        import csv
        import cStringIO
        self._setup_project('csv_project',
                                      'csv_file',
                                      dict(name='English', code='en'))
...
class TestChangeValue(TestMultiprojectPoTranslate):
    ...
    def test_change_value_after_import_in_one_project(self):
        ...
        first_mes = get_message_list(self.proj.id, self.proj_lang.id)
        second_mes = get_message_list(self.new_project.id, self.proj_lang.id)
...
class TestSectionTwoProjecs(TestMultiprojectPoTranslate):
    source_messages = [{'mes.sag.e1': 'message1',
                        'mes.sag.e2': 'message2'}]
    target_messages = [{'mes.sag.e1': 'message1',
                        'mes.sag.e2': 'message2'}]
    ...
    def test_sections(self):
        sect, subsect = get_sections_info(self.proj.id, self.proj_lang.id)
...
class TestPermissionPoTrans(TestPermissions):
    ...
    def test_list_with_perms(self):
        perms = get_all_permissions(self.proj.id)
        for row in perms:
...
class TestSaveNewMeaningSimple(TestPoTranslate):
    ...
    def test_save_new_meaning_for_old_set_message(self):
        ...
        self.assertEqual(set(i.msgstr for i in target_message_versions),
                         set(['ru_message3']))
